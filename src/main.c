 ...
 * References and Acknowledgments: ChatGPT
 * provided a sorting method for our files in
 * the ls program (details in utils/ls.c)*/
...
{
  FILE *s = NULL;
  if (!get_args (argc, argv, &s))
    usage ();
  if (s == NULL)
    {
      // Opening the shell with STDIN if the file was not provided
      shell (stdin);
    }
  else
    {
      // Opening the shell with the provided file otherwise
      shell (s);
    }
  return EXIT_SUCCESS;
}
